...
> Which agentic scenarios have draft fleet skill specs? For each, note whether the required API capabilities exist yet or are being built as part of this PRD. This section makes fleet enablement part of the definition of done — a feature is not complete until a fleet agent can do it too (Shelby principle).

| Skill Name | Scenario | API Capabilities Ready? | Status | Notes |
|------------|----------|------------------------|--------|-------|
| `{skill-name}` | {maps to scenario N} | Yes / No / Partial | Draft / In Build / Live | |

{If `fleet-skill-drafts.md` exists in the package folder, pre-populate this table from it. If not, leave blank — fleet skill drafts are generated by the agentic-api-designer skill.}
...
| Fleet Skill Drafts | {link} | PM-authored draft fleet agent skill specs |
