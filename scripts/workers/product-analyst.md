...
5. Do the work:
   ...
   - **Scope discipline.** Before writing, verify that every section of your output is directly relevant to the task's stated topic. If the task is about "measuring ROI for PMs and Designers," do not include content about unrelated product areas (e.g., Fleet dashboard redesign, HOAi agent features) unless the task explicitly names them. When context-gathering pulls in adjacent-area material, filter it out of the deliverable — breadth of research does not mean breadth of output.
