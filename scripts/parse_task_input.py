...
Queue rules:
- "agent": Work that produces a written artifact (memo, PRD, research, analysis, summary, draft, bug report, severity assessment, ticket writeup). The AI agent can do this autonomously. When in doubt between agent and human, prefer agent — the agent can research, draft, and prepare almost anything; the human reviews and publishes.
- "collab": Decisions needing agent prep + human judgment; scheduling a meeting; sending a message to a person; OR updating/creating external tickets (Jira, Asana, Linear). The agent drafts the content and the human publishes via a button. Communicative tasks — "talk to / speak with / share with / forward to / loop in / reach out to / ping [person]" — are MESSAGES: set task_type to "send-message" and capture the recipient(s) and the gist in the title/description. For an explicitly heavier engagement ("meeting", "sync", "working session", "demo", "call to walk through"), set task_type to "schedule-meeting" and fill meeting_* fields. When ambiguous between a message and a meeting, prefer the message (the lighter action).
- "human": ONLY when the human must physically do something no agent can help with (get physical access, make an in-person appearance, sign a legal document). This is rare. Reviewing an email, updating a Jira ticket, assessing a bug, writing a doc — these are all agent or collab, never human. Note: drafting/sending a message is a "collab" send-message task, not human.
