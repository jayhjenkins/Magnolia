...
description: Use when computing or validating the operator's Q2 2026 Rocks metrics (Home WAU and Board Member Weekly Login Rate) - defines exact, reproducible Pendo query specifications and aggregation rules so weekly runs are byte-for-byte comparable across time
...
Lock the calculation methodology for the operator's two Q2 2026 EOS Rocks so weekly reporting is reproducible and trend lines remain comparable across time. This skill is the single source of truth for **how** these metrics are computed; it does not handle persistence or reporting (the `/update-rocks` command does that).
...
| **Pendo subId** | resolve via `python3 scripts/profile_lib.py --pendo-subid` — never embed a literal | Required on every MCP call |
...
mcp__claude_ai_Pendo__activityQuery(
  subId="{subId}",  # resolve via `python3 scripts/profile_lib.py --pendo-subid`
...
| Querying without `subId` | Every Pendo MCP call requires `subId`, resolved via `python3 scripts/profile_lib.py --pendo-subid`. |
